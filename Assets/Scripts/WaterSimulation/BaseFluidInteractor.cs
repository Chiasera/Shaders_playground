...
namespace Fusion.Fluid
{
    ...
    public abstract class BaseFluidInteractor : MonoBehaviour
    {
        ...
        //Awake is called before OnEnable so anything happening here will execute even is the script is disabled upon initialization
        public void Awake()
        {
            rndTimeOffset = new float[6]; 

            for (int i = 0; i < 6; i++) //define random parameters for the turbulence
            {
        ...
        public Vector2 GenerateDrift() {
            return Vector2.zero; //fluid.GetWaveDirection() * fluid.GetWaveSpeed()/50;
        }
